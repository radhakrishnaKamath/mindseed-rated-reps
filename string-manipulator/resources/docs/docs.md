...
The `home-routes` handler in the `string-manipulator.routes.home` namespace
...
The `home-page` function will in turn call the `string-manipulator.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/string_manipulator/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `string-manipulator.handler` namespace:
...
Request middleware functions are located under the `string-manipulator.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `string-manipulator.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `string-manipulator.core` in some other namespace.
