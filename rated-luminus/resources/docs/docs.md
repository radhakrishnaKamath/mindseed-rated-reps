...
The `home-routes` handler in the `rated-luminus.routes.home` namespace
...
The `home-page` function will in turn call the `rated-luminus.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `rated-luminus.handler` namespace:
...
Request middleware functions are located under the `rated-luminus.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `rated-luminus.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `rated-luminus.core` in some other namespace.
