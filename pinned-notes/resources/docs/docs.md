...
The `home-routes` handler in the `pinned-notes.routes.home` namespace
...
The `home-page` function will in turn call the `pinned-notes.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/pinned_notes/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `pinned-notes.handler` namespace:
...
Request middleware functions are located under the `pinned-notes.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `pinned-notes.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `pinned-notes.core` in some other namespace.
